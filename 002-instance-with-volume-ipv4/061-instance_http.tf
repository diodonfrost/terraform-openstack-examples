...
resource "openstack_compute_instance_v2" "http2" {
  name        = "web02"
  flavor_name = var.flavor_http
  key_pair    = openstack_compute_keypair_v2.user_key.name
  user_data   = file("scripts/web02.sh")
  
  network {
	    port = openstack_networking_port_v2.http2_int_v4.id
  }

# Install system in volume

  block_device {
    volume_size           = var.volume_http2
...
resource "openstack_networking_port_v2" "http2_int_v4" {
  name           = "port-instance-http2"
